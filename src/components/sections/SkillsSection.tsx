...
export function SkillsSection({ id }: { id?: string }) {
  ...
  const renderSkillCategory = (categoryKey: string) => {
    ...
    return (
      <Card
        key={categoryKey}
        variant="base"
        className="p-5 sm:p-6 hover:border-primary-500/30 transition-colors border border-gray-900 dark:border-transparent"
      >
            ...
            >
              {skill.icon ? (
                <skill.icon
                  ...
                  style={{
                    color: brandColors[skill.name] || 'var(--text-secondary)',
                  }}
                />
              ) : (
  ...
  return (
    <section
      id={id}
      className="py-16 sm:py-20 md:py-24 bg-slate-50 dark:bg-background overflow-hidden"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  className={cn(
                    'relative px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-medium transition-all duration-200 cursor-pointer z-10',
                ...
                >
                  ...
                  {activeTab === tab.id && shouldReduce && (
                    <div className="absolute inset-0 bg-blue-600 rounded-lg -z-10 shadow-md" />
                  )}
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {activeTab === 'fullstack' ? (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-7 md:gap-8">
                    {activeTabConfig.categories.map(categoryKey =>
                      renderSkillCategory(categoryKey)
                    )}
                  </div>
                )}
              </motion.div>
              ...
              <div className="relative rounded-xl overflow-hidden p-[1px]">
                ...
                <div className="relative bg-white dark:bg-[#0a0e1a] border border-transparent dark:border-white/10 rounded-xl p-6 sm:p-8 text-center shadow-sm">
                  ...
                  <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 max-w-xl mx-auto mb-4 leading-relaxed">
                    Specializing in AI-enabled Full Stack ecosystems, Machine
                    Learning pipelines, and Multi-agent AI applications —
                    bridging backend reliability with intelligent automation.
                  </p>
                  <div className="flex flex-wrap justify-center gap-2">
                    {['React', 'Next.js', 'Node.js', 'LLM APIs', 'Django'].map(
                      skill => (
                        <span
                          key={skill}
                          className="px-3 py-1 text-xs sm:text-sm rounded-lg bg-blue-50 dark:bg-white/5 text-blue-700 dark:text-blue-100 font-medium"
                        >
                          {skill}
                        </span>
                      )
