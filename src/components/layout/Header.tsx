...
export function Header() {
  ...
  return (
    <motion.header
      ...
      style={{
        boxShadow: isScrolled ? '0 4px 32px rgba(0,0,0,0.12)' : 'none',
        ...
        {isMenuOpen && (
          <div className="lg:hidden">
            ...
            <motion.nav
              ...
              id="mobile-menu"
              className="fixed top-20 left-0 right-0 z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-b border-white/20 dark:border-white/10 shadow-2xl"
              ...
              <div className="px-5 py-6 space-y-4">
                {navigationItems.map((item, index) => (
                  <motion.button
                    ...
                    transition={{ delay: shouldReduce ? 0 : index * 0.05 }}
                    whileHover={{ scale: 1.04, y: -2 }}
                    whileTap={{ scale: 0.96 }}
                    key={item.href}
                    onClick={() => handleSmoothScroll(item.href)}
